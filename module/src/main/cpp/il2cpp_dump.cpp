...
//    outPut << "\n// Namespace: " << il2cpp_class_get_namespace(klass) << "\n";  // cannot get namespace of some enum
    ...
    // attribute
    Il2CppCustomAttrInfo *attrInfo = il2cpp_custom_attrs_from_class(klass);
    Il2CppArray *attrList = il2cpp_custom_attrs_construct(attrInfo);
    il2cpp_custom_attrs_free(attrInfo);
    size_t attr_count = il2cpp_array_length(attrList);
    if(attr_count > 0){
        auto items = attrList->vector;
        for (int i=0; i<attrList->max_length; i++) {
            auto *attr_obj = (Il2CppObject *) items[i];
            if(attr_obj) {
                outPut << "[" << format_attributes(attr_obj) << "]\n";
            }
        }
    }
